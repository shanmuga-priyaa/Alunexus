...
function login2(){
	...
	function signup(){
		...
		// Insert data into alumnus_bio table
		$save_bio = $this->db->query("INSERT INTO alumnus_bio (firstname, middlename, lastname, gender, role, batch, course_id, email, connected_to, status, date_created) VALUES ('$firstname', '$middlename', '$lastname', '$gender', '$role', '$batch', '$course_id', '$email', '$connected_to', 0, NOW())");
		if(!$save_bio){
			return 0; // Error saving alumnus_bio data
		}
		...
		$save_user = $this->db->query("INSERT INTO users (name, username, password, type, auto_generated_pass, alumnus_id) VALUES ('$firstname $middlename $lastname', '$email', '$hashed_password', 3, 0, '$alumnus_id')");
		if(!$save_user){
			return 0; // Error saving user data
		}
		...
		$save_alumnus = $this->db->query("INSERT INTO user (firstname, middlename, lastname, gender, email, password, date_created, date_updated, alumnus_id) VALUES ('$firstname', '$middlename', '$lastname', '$gender', '$email', '$hashed_password', NOW(), NOW(), '$alumnus_id')");
		if(!$save_alumnus){
