                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <button class="btn btn-sm btn-outline-success verify_career" type="button" data-id="<?php echo $row['id'] ?>">Verify</button>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
...
<script>
    ...
    $('.verify_career').click(function(){
        var id = $(this).attr('data-id');
        verify_career(id);
    });

    function verify_career(id){
        start_load()
        $.ajax({
            url:'ajax.php?action=verify_career',
            method:'POST',
            data:{id:id},
            success:function(resp){
                if(resp==1){
                    alert_toast("Job successfully verified",'success')
                    setTimeout(function(){
                        location.reload()
                    },1500)
                } else {
                    alert_toast("Failed to verify job",'error')
                }
            }
        })
    }
