...
Class Actions extends DBConnection{
    ...
    function save_user(){
        ...
        @$check= $this->conn->query("SELECT * FROM user where `email` = '{$email}' ".($id > 0 ? " and id != '{$id}' " : ""))->num_rows;
        if(@$check> 0){
            $resp['status'] = 'failed';
    ...
    function delete_user(){
        ...
        @$delete = $this->conn->query("DELETE FROM `user` where id = '{$id}'");
        if($delete){
            $resp['status']='success';
    ...
    function update_credentials(){
        ...
        if(!empty($password) && md5($old_password) != $_SESSION['password']){
            ...
            $sql = "UPDATE `user` set {$data} where id = '{$_SESSION['id']}'";
            @$save = $this->conn->query($sql);
            if($save){
                $resp['status'] = 'success';
                $_SESSION['flashdata']['type'] = 'success';
    ...
    function find_user(){
        extract($_POST);
        $sql = "SELECT *,CONCAT(firstname,' ',middlename,' ',lastname) as `name` FROM `user` where (CONCAT(firstname,' ',middlename,' ',lastname) LIKE '%{$keyword}%' OR CONCAT(lastname,' ',firstname,' ',middlename) LIKE '%{$keyword}%' OR email LIKE '%{$keyword}%') and id != '{$_SESSION['id']}'";
